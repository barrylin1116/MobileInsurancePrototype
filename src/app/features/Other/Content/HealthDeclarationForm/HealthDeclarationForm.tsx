...
import { updateRequiredDocumentsList } from '../../../../store/common/actions';
...
const HealthDeclarationForm = React.forwardRef<any, any>((props, ref) => {
  ...
  const getHealthDeclarationFormQA = ['是否領有身心障礙手冊或身心障礙證明(如勾選是者，請提供前述手冊或證明。)', '被保險人有無投保其他商業實支實付型傷害醫療保險？', '被保險人有無投保其他商業實支實付型醫療保險？', '過去二年內是否曾因接受健康檢查有異常情形而被建議接受其他檢查或治療？(亦可提供檢查報告代替回答)',
  ...
  const diseaseQuestionnaireList = [
    { title: '一般疾病問卷', key: 'GeneralDiseaseQuestionnaire' },
    { title: '甲狀腺疾病問卷', key: 'ThyroidDiseaseQuestionnaire' },
    { title: '肝炎或肝炎病毒帶原問卷', key: 'HepatitisOrHepatitisCarrierQuestionnaire' },
    { title: '泌尿系統疾病問卷', key: 'UrinarySystemDiseaseQuestionnaire' },
    { title: '食道、胃或十二指腸潰瘍問卷', key: 'EsophagusStomachQuestionnaire' },
    { title: '氣喘/慢性支氣管炎/慢性阻塞性肺疾病問卷', key: 'BronchialDiseaseQuestionnaire' },
    { title: '高血壓問卷', key: 'HypertensionQuestionnaire' },
    { title: '高尿酸血症/痛風/關節炎問卷', key: 'HyperuricemiaQuestionnaire' },
    { title: '糖尿病問卷', key: 'DiabetesQuestionnaire' },
    { title: '癲癇問卷', key: 'EpilepsyQuestionnaire' },
    { title: '心臟疾病問卷', key: 'HeartDiseaseQuestionnaire' },
    { title: '腫瘤及息肉問卷', key: 'TumorAndPolypQuestionnaire' },
    { title: '小兒麻痹問卷', key: 'PoliomyelitisQuestionnaire' },
    { title: '外傷骨骼問卷', key: 'TraumaAndSkeletalInjuryQuestionnaire' },
    { title: '眼部疾病問卷', key: 'EyeDiseaseQuestionnaire' }
  ];
  ...
  const handleCheckboxChange = (event: any, index: number) => {
    ...
    console.log(diseaseQuestionnaireList[index]);
    const newRequiredDocumentsList = requiredDocumentsList2.map(
      (data: any) => {
        return data.key === diseaseQuestionnaireList[index].key
          ? {
              ...data,
              visible: checked
            }
    ...
    reduxDispatch(updateRequiredDocumentsList(
      {
        requiredDocumentsList: newRequiredDocumentsList
      }
    ));
    if (checked) {
        ...
        return (
          index % 2 === 0 && (
            <div
              ...
              key={`qa-${index}`} className="row form-row m-0 p-2 border elife-border-color-1"
            >
  ...
  return (
    <>
      <div className="container">
        <div className="row m-0 p-4 border elife-border-color-1">
          <div className="col-6">
          ...
          }
        {
              checkboxesState.filter(data => data).length > 0 && (
                <>
                  <div
                    className="row m-0 p-2 border elife-border-color-1"
                    style={{ background: '#edf5f4', borderTop: 'none' }}
                  >
                    <div className="col-12">
