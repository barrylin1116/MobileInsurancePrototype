...
const Header: React.FC = () => {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setTimeMillSecond(timeMillSecond - 1 <= 0 ? 0 : timeMillSecond - 1);
    }, 1000);
  ...
  return (
    <header>
      ...
      <nav className="navbar fixed-top navbar-expand">
        ...
        <div className="collapse navbar-collapse justify-center col-6">
          <div className="d-flex">
            <h2 className="section-title" style={{ fontSize: '1.2rem', color: ' white' }}>
              <img
                style={{ width: '27px', marginRight: '3px' }} src={require('assets/img/icons/ois-logo.svg').default}
                alt="logo-台銀人壽"
              />
              臺銀人壽行動投保
            </h2>
        ...
        <div className="d-flex col-3">
          <div className="greeting nav-item-wrapper">
            <div className="nav-item pe-0 pl-40x">
              <div className="refreshInfo d-flex flex-row">
          ...
          <div className="greeting nav-item-wrapper">
            ...
            <div className="sub-items">
              <div
                className="item" title="重置密碼" role="button"
                onClick={() => routerHistory.push(ROUTES.PAGE_SETTINGS_PASS_PHRASE_AUTH)}
              >重置密碼
              </div>
              <div
                className="item" title="登入記錄" role="登入記錄"
                onClick={() => setRecordModalVisible(true)}
              >登入記錄
              </div>
              <div
                className="item" title="系統登出" role="button"
